...
export default function WatchersPage() {
  ...
  const [data, { refetch }] = createResource(
    ...
    async () => {
      ...
      const [watchers, sources] = await Promise.all([api.list(), api.sources()])
      return { watchers, sources }
    },
  )
  const connect = () => {
    void dialog.push(() => (
      <DialogConnectSource sources={data()?.sources ?? []} onSaved={() => void refetch()} />
    ))
  }
    ...
    <main class="h-full min-h-0 overflow-auto bg-v2-background-bg-deep text-v2-text-text-base">
      <div class="mx-auto flex w-full max-w-6xl flex-col gap-6 px-5 py-8 md:px-10">
        <header class="flex items-start justify-between gap-4">
          ...
          <ButtonV2 icon="plus-small" disabled={!data()?.sources.some((source) => source.connection)} onClick={() => navigate(href("/new"))}>
            New watcher
          </ButtonV2>
        </header>
              ...
              <Show when={current().watchers.length} fallback={<FirstRun kind="watcher" href={href("/new")} />}>
                <section class="overflow-hidden rounded-xl border border-v2-border-border-base bg-v2-background-bg-base">
                  <For each={current().watchers}>
                    {(watcher, index) => {
                      const source = () => current().sources.find((item) => item.integration.id === watcher.integrationID)
                      return (
                        <div classList={{ "border-t border-v2-border-border-base": index() > 0 }} class="flex flex-col gap-3 p-4 md:flex-row md:items-center">
                          <A href={href(`/${watcher.id}`)} class="min-w-0 flex-1 rounded-md outline-none focus-visible:ring-2 focus-visible:ring-v2-border-border-focus">
                            <div class="flex items-center gap-2">
                              <span class="flex size-7 items-center justify-center rounded-md bg-v2-background-bg-surface text-12-medium uppercase text-v2-text-text-muted">
                                {source()?.integration.name.slice(0, 1) ?? "?"}
                              </span>
                              <span class="truncate text-14-medium text-v2-text-text-strong">{watcher.name}</span>
                              <span class="rounded-full bg-v2-background-bg-surface px-2 py-0.5 text-11-regular text-v2-text-text-muted">
                                {watcher.action.mode === "run" ? "Auto-run" : watcher.action.mode === "awaiting_run" ? "Awaiting run" : "Inbox"}
                              </span>
                            </div>
                            <p class="mt-2 truncate text-12-regular text-v2-text-text-muted">
                              {criteriaSummary(watcher.criteria)} · {routingSummary(watcher.routing)}
                            </p>
                            <Show when={watcher.lastError}>
                              <p class="mt-1 text-12-regular text-v2-text-text-danger">Paused: {watcher.lastError}</p>
                            </Show>
                          </A>
                          <div class="flex shrink-0 items-center gap-3">
                            <span class="text-12-regular text-v2-text-text-muted">
                              {watcher.lastRunAt ? `Last run ${new Date(watcher.lastRunAt).toLocaleString()}` : "Never run"}
                            </span>
                            <button
                              type="button"
                              role="switch"
                              aria-checked={watcher.enabled}
                              aria-label={`${watcher.enabled ? "Disable" : "Enable"} ${watcher.name}`}
                              classList={{ "bg-v2-background-bg-interactive": watcher.enabled, "bg-v2-background-bg-surface": !watcher.enabled }}
                              class="relative h-6 w-10 rounded-full border border-v2-border-border-base transition-colors"
                              onClick={() => void toggle(watcher)}
                            >
                              <span classList={{ "translate-x-4": watcher.enabled, "translate-x-0": !watcher.enabled }} class="absolute left-0.5 top-0.5 size-4 rounded-full bg-white shadow transition-transform" />
                            </button>
                          </div>
                        </div>
                      )
...
export function WatcherEditorPage() {
  ...
  const [loaded] = createResource(
    ...
    async () => {
      const api = serverSdk().nextApi.issueWatchers
      const [sources, settings, watcher] = await Promise.all([
        api.sources(),
        api.getSettings(),
