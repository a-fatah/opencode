    ...
    return Effect.gen(function* () {
      ...
      yield* tx.run(`
        CREATE TABLE \`issue_materialization\` (
          ...
          \`action\` text DEFAULT '{"mode":"inbox","promptTemplate":"{{issue.key}} {{issue.title}}","writeback":{"comment":false,"commentOnFailure":false}}' NOT NULL,
          \`prompt\` text,
      ...
      yield* tx.run(`
        CREATE TABLE \`session_provenance\` (
          ...
          \`branch\` text,
          \`writeback\` text DEFAULT '{}' NOT NULL,
