...
export const IssueMaterializationTable = sqliteTable(
  "issue_materialization",
  {
    ...
    action: text({ mode: "json" }).$type<IssueWatcher.Action>().notNull().default({
      mode: "inbox",
      promptTemplate: "{{issue.key}} {{issue.title}}",
...
export const SessionProvenanceTable = sqliteTable(
  "session_provenance",
  {
    ...
    branch: text(),
    writeback: text({ mode: "json" }).$type<IssueWatcher.WritebackPlan>().notNull().default({}),
